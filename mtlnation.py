...
def obtenernumpagurl(url):
    ...
    driver.close()
    tit = soup.find_all(class_='selectable')
    tituloen = str(tit[0].get_text()).strip()
    titulo = quitar_simbolos_especiales(ts.google(tituloen, to_language='es'))
    print(titulo)
    autor = 'Cool late summer'
    descrip = soup.find_all(class_='chapters')
    # descripsum = descrip[0].find_all(class_='summary')
    descripcion = ts.google(descrip[0].get_text().strip(), to_language='es')
    print(descripcion)
...
def main():
    ...
    global genero
    links = [
        'https://mtlnation.com/novel/become-a-villain-in-a-book-and-the-female-leads-designs-collapsed',
    ]
